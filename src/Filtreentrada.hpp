...
public:
    ...
    virtual void config(int iPin, unsigned long minMillis);
    int getValorRaw();
    void update();
...
public:
    ...
    void config(int iPin, unsigned long minMillis, int llindar) {
        FiltreEntrada::config(iPin, minMillis);
        _llindar = llindar;
    }
...
public:
    ...
    void config(int iPin, unsigned long minMillis) {
        FiltreEntrada::config(iPin, minMillis);
    ...
    bool _rawToBool () {
        _valorRaw = 0;
        return(digitalRead(_inputPin) == 0);
    }    
